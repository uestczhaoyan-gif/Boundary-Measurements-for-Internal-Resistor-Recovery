...
def unchanged_sparse_loss(pred_delta, y_change):
    mask = y_change < 0.5
    if not bool(mask.any()):
        return torch.tensor(0.0, device=pred_delta.device, dtype=pred_delta.dtype)
    return pred_delta[mask].abs().mean()


def unchanged_hinge_loss(pred_delta, y_change, threshold):
    mask = y_change < 0.5
    if not bool(mask.any()):
        return torch.tensor(0.0, device=pred_delta.device, dtype=pred_delta.dtype)
    err = torch.relu(pred_delta[mask].abs() - threshold)
    return (err * err).mean()


def false_positive_sep_loss(pred_delta, y_change, margin):
    abs_pred = pred_delta.abs()
    big_pos = torch.full_like(abs_pred, 1e6)
    big_neg = torch.full_like(abs_pred, -1e6)

    min_true = torch.where(y_change > 0.5, abs_pred, big_pos).min(dim=1).values
    max_nontrue = torch.where(y_change < 0.5, abs_pred, big_neg).max(dim=1).values
    return torch.relu(margin - (min_true - max_nontrue)).mean()
...
def evaluate(model, loader, device, args, coords=None, phys_scale=1.0):
    ...
    with torch.no_grad():
        for xb, xrawb, ycb, ydb, tids, _tvals in loader:
            ...
            loss_sparse = unchanged_sparse_loss(pred, ycb)
            loss_hinge = unchanged_hinge_loss(pred, ycb, args.hinge_threshold)
            loss_sep = false_positive_sep_loss(pred, ycb, args.sep_margin)
...
def run(args):
    ...
    metrics = {
        ...
        "loss_weights": {
            ...
            "lambda_hinge": float(args.lambda_hinge),
            "lambda_sep": float(args.lambda_sep),
            "hinge_threshold": float(args.hinge_threshold),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Change3 重建模型 v2_new（加权回归 + 第4假阳性抑制）")
    parser.add_argument("--data-path", default=str(script_dir.parent / "data_fixed" / "training_data64_fixed_3.csv"))
    parser.add_argument("--cache-path", default=str(script_dir / "cache_change3_v2_new.npz"))
    ...
    parser.add_argument("--lambda-hinge", type=float, default=0.10)
