...
BASE_R = 1000.0
EXCITATIONS = 32
KNOWN_CHANGE_COUNT = 3
LEGACY_STEM = "training_data64_fixed_3"
...
def default_change3_dataset_tag(data_path):
    stem = Path(data_path).stem
    if stem == LEGACY_STEM:
        return "5mA"
    prefix = f"{LEGACY_STEM}_"
    if stem.startswith(prefix):
        return sanitize_dataset_tag(stem[len(prefix):])
    return sanitize_dataset_tag(stem)


def resolve_change3_runtime_paths(args, script_dir, default_cache_name):
    default_data_path = script_dir.parent / "data_fixed" / f"{LEGACY_STEM}.csv"
...
def soft_count3_loss(pred_delta, threshold=50.0, temp=12.0, target_count=KNOWN_CHANGE_COUNT):
    soft_count = torch.sigmoid((pred_delta.abs() - threshold) / temp).sum(dim=1)
    target = torch.full_like(soft_count, float(target_count))
    return torch.abs(soft_count - target).mean()


def hardest_sep_loss(pred_delta, y_change, margin=10.0):
    abs_pred = pred_delta.abs()
    pos_scores = abs_pred.masked_fill(y_change < 0.5, float("inf"))
    neg_scores = abs_pred.masked_fill(y_change > 0.5, float("-inf"))
    min_pos = pos_scores.min(dim=1).values
    max_neg = neg_scores.max(dim=1).values
    return torch.relu(margin - (min_pos - max_neg)).mean()
...
def evaluate(model, loader, device, huber, args, coords=None):
    model.eval()
    val_loss = 0.0
    sum_all_abs = 0.0
    n_all = 0
    ...
    with torch.no_grad():
        for xb, _xraw, ycb, ydb, tids, _tvals in loader:
            ...
            loss_count3 = soft_count3_loss(
                pred,
                threshold=args.count_threshold,
                temp=args.count_temp,
                target_count=KNOWN_CHANGE_COUNT,
            )
            loss_sep = hardest_sep_loss(pred, ycb, margin=args.sep_margin)
            if coords is not None and args.lambda_coord > 0:
                loss_coord = coord_moment_loss(pred, ycb, coords, temp=args.coord_temp)
            else:
            ...
            loss = (
                ...
                + args.lambda_count3 * loss_count3
                + args.lambda_sep * loss_sep
                + args.lambda_coord * loss_coord
            )
    ...
    score = (
        ...
        + args.score_sparse * abs(avg_gt50 - float(KNOWN_CHANGE_COUNT))
        + args.score_top3 * (1.0 - top3_precision)
    )
    return {
        "loss": val_loss,
        "mae_all": mae_all,
...
def run(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for xb, xrawb, ycb, ydb, _tids, _tvals in tr_loader:
            ...
            if ep >= args.count_start_epoch:
                ...
                loss_count3 = torch.tensor(0.0, device=device)
            if ep >= args.sep_start_epoch:
                loss_sep = hardest_sep_loss(pred, ycb, margin=args.sep_margin)
            else:
        ...
        val = evaluate(model, va_loader, device, huber, args, coords=coords)
        ...
        if ep == 1 or ep % 5 == 0:
            print(
                ...
                f"| val_top3_precision={val['top3_precision']:.4f}"
            )
    ...
    test = evaluate(model, te_loader, device, huber, args, coords=coords)
    ...
    metrics = {
        ...
        "coord": {
            ...
            "coord_temp": float(args.coord_temp),
        },
        "count3": {
            "lambda_count3": float(args.lambda_count3),
            "count_threshold": float(args.count_threshold),
            "count_temp": float(args.count_temp),
            "count_start_epoch": int(args.count_start_epoch),
        },
        "separation": {
            "lambda_sep": float(args.lambda_sep),
            "sep_margin": float(args.sep_margin),
            "sep_start_epoch": int(args.sep_start_epoch),
        },
        "score": {
            "score_mae_all": float(args.score_mae_all),
            "score_sparse": float(args.score_sparse),
            "score_top3": float(args.score_top3),
        },
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Change3 reconstruction with count3 + hardest-separation losses (v2_coord).")
    ...
    parser.add_argument("--score-top3", type=float, default=10.0)
    parser.add_argument("--lambda-coord", type=float, default=0.12)
    parser.add_argument("--coord-temp", type=float, default=20.0)
    parser.add_argument("--lambda-count3", type=float, default=0.25)
    parser.add_argument("--count-threshold", type=float, default=52.0)
    parser.add_argument("--count-temp", type=float, default=12.0)
    parser.add_argument("--count-start-epoch", type=int, default=10)
    parser.add_argument("--lambda-sep", type=float, default=0.30)
    parser.add_argument("--sep-margin", type=float, default=12.0)
    parser.add_argument("--sep-start-epoch", type=int, default=15)
