...
def order_loss(pred, y_change, t_on=70.0, t_off=50.0, margin=12.0, w_k2=1.25, w_k3=1.45):
    ...
    if m0.any():
        loss[m0] = torch.relu(s0[m0] - t_off)
    if m1.any():
        loss[m1] = torch.relu(t_on - s0[m1]) + torch.relu(s1[m1] - t_off)
    if m2.any():
        gap_term = torch.relu(margin - (s1[m2] - s2[m2]))
        loss[m2] = w_k2 * (torch.relu(t_on - s1[m2]) + torch.relu(s2[m2] - t_off) + gap_term)
    if m3.any():
        gap_term = torch.relu(margin - (s2[m3] - s3[m3]))
        loss[m3] = w_k3 * (torch.relu(t_on - s2[m3]) + torch.relu(s3[m3] - t_off) + gap_term)
    return loss.mean()
...
def evaluate_val(model, loader, device, huber, l1, args, ep):
    ...
    with torch.no_grad():
        for xb, ycb, ydb in loader:
            ...
            loss_order = order_loss(
                pred,
                ycb,
                t_on=args.order_t_on,
                t_off=args.order_t_off,
                margin=args.order_margin,
                w_k2=args.order_weight_k2,
                w_k3=args.order_weight_k3,
            )
            ...
            k23_mask = true_count >= 2
            if k23_mask.any():
                overpredict_k23_list.extend(overpredict[k23_mask].cpu().tolist())
    ...
    val_avg_overpredict_k23 = float(np.mean(overpredict_k23_list)) if overpredict_k23_list else 0.0
    val_score = (
        val_mae_changed
        + args.val_mae_all_alpha * val_mae_all
        + args.val_sparse_alpha * val_avg_gt
        + args.val_overpredict_alpha * val_avg_overpredict
        + args.val_overpredict_k23_alpha * val_avg_overpredict_k23
    )
    return (
        val_loss,
        val_mae_all,
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if ep % args.log_every == 0 or ep == 1:
            ...
            print(
                ...
                f"| val_overpredict_k23={va_avg_overpredict_k23:.3f}"
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes MLP regression modelo5 (MAE-first + order loss).")
    ...
    parser.add_argument("--lambda-count", type=float, default=0.01)
    parser.add_argument("--lambda-order", type=float, default=0.12)
    parser.add_argument("--order-t-on", type=float, default=70.0)
    parser.add_argument("--order-t-off", type=float, default=50.0)
    parser.add_argument("--order-margin", type=float, default=0.0)
    parser.add_argument("--order-weight-k2", type=float, default=1.0)
    parser.add_argument("--order-weight-k3", type=float, default=1.0)
    ...
    parser.add_argument("--val-overpredict-k23-alpha", type=float, default=0.0)
    ...
    parser.add_argument("--count-cls2-weight", type=float, default=0.35)
...
if __name__ == "__main__":
    ...
    resolve_dataset_runtime_paths(args, script_dir, "./cache_dataset_reg.npz")
    run(args)
