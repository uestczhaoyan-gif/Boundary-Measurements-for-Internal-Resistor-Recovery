...
def main():
    parser = argparse.ArgumentParser(description="Inference for 64Nodes MLP regression modelo5.")
    ...
    resolve_inference_runtime_paths(args, script_dir, "./cache_dataset_reg.npz")
    print(
        "[Runtime] "
    ...
    for idx in sample_idx:
        ...
        with torch.no_grad():
            pred_delta = model(xb).cpu().numpy().flatten()
        ...
        pred_change_values = [float(pred_delta[i]) for i in pred_change_ids]
        true_change_values = [float(true_delta[i]) for i in true_change_ids]
        results.append(
            {
                "index": int(idx),
                ...
                "pred_change_deltas": pred_change_values,
                "true_change_ids": true_change_ids,
                "true_change_deltas": true_change_values,
