...
def run(args):
    ...
    with torch.no_grad():
        for xb, ycb, ydb in test_loader:
            ...
            pred_counts.extend((pred.abs() > args.hinge_threshold).sum(dim=1).cpu().tolist())
            true_counts.extend((ycb > 0.5).sum(dim=1).cpu().tolist())

    mae_all /= max(n_all, 1)
    mae_changed = mae_changed / max(n_changed, 1)
    avg_gt50 = float(np.mean(pred_counts)) if pred_counts else 0.0
    ...
    print(f"avg(|dR|>{args.hinge_threshold:.1f})={avg_gt50:.2f}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes MLP regression baseline.")
    ...
    parser.add_argument("--w-change", type=float, default=1.8)
    parser.add_argument("--w-unchange", type=float, default=1.2)
    parser.add_argument("--lambda-hinge", type=float, default=0.2)
