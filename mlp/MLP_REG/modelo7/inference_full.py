...
def main():
    parser = argparse.ArgumentParser(description="Combined inference for MLP CLS+REG using top-K from REG, where K comes from CLS.")
    ...
    parser.add_argument("--reg-cache-path", default="./cache_dataset_reg_mixer.npz")
    ...
    parser.add_argument("--cls-dir", default="../../MLP_CLS/modelo7")
    ...
    parser.add_argument("--reg-trunk-dim", type=int, default=256)
    ...
    parser.add_argument("--cls-trunk-dim", type=int, default=256)
    ...
    reg_cache = resolve_default_artifact_path(script_dir / "cache", Path(args.reg_cache_path).name, dataset_tag, data_path) if args.reg_cache_path == "./cache_dataset_reg_mixer.npz" else Path(args.reg_cache_path)
    ...
    cls_cache = resolve_default_artifact_path(cls_dir / "cache", "cache_dataset_cls_mixer.npz", dataset_tag, data_path) if not args.cls_cache_path else Path(args.cls_cache_path)
    ...
    MLPMixerClassifier = load_module_attr("mlp_cls_modelo7_model", cls_dir / "model" / "model.py", "MLPMixerClassifier")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    reg_model_obj = MLPMixerRegressor(
        num_excitations=x_reg.shape[1],
        in_dim=x_reg.shape[2],
        hidden_dim=args.reg_hidden_dim,
        trunk_dim=args.reg_trunk_dim,
        out_dim=112,
    ...
    cls_model_obj = MLPMixerClassifier(
        num_excitations=x_cls.shape[1],
        in_dim=x_cls.shape[2],
        hidden_dim=args.cls_hidden_dim,
        trunk_dim=args.cls_trunk_dim,
