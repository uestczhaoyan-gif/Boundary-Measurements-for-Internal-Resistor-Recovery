...
def search_aux23_threshold_weighted(main_probs, aux23_probs, true_labels, main_thrs, step=0.01, penalty_32=0.12, bonus_r3=0.06, bonus_r2=0.05):
    best_thr = 0.5
    best_f = -1.0
    best_score = -1e9
...
def run(args):
    ...
    best_aux_epoch = 0
    bad_aux_epochs = 0
    stage2_epochs = min(args.stage2_epochs, max(args.epochs - stage1_epochs, 0))
    thrs_tensor = torch.tensor(main_thrs, dtype=torch.float32, device=device).view(1, -1)
    t3 = float(main_thrs[2])
    ...
    for ep2 in range(1, stage2_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            logits_main, logits_aux = model(xb)
            with torch.no_grad():
                main_probs = torch.sigmoid(logits_main / args.temp)
                pred_main = (main_probs > thrs_tensor).sum(dim=1)
                p3 = main_probs[:, 2]
                boundary_mask = torch.abs(p3 - t3) < args.boundary_margin
                hard_mask = (yb >= 2) | (pred_main >= 2) | boundary_mask
            if not hard_mask.any():
                continue
            target_aux = (yb[hard_mask] > 2).float()
            target_aux = target_aux * (1.0 - args.aux_label_smoothing) + 0.5 * args.aux_label_smoothing
            loss_aux = F.binary_cross_entropy_with_logits(logits_aux[hard_mask], target_aux)
            opt_aux.zero_grad()
            loss_aux.backward()
            if args.grad_clip > 0:
            ...
            used_samples += int(hard_mask.sum().item())
        ...
        if ep2 % args.log_every == 0 or ep2 == 1 or ep2 == stage2_epochs:
            ...
            print(
                ...
                f"| val_macro_f1={fused_f1:.4f} | aux_thr={aux_thr:.2f} | hard_samples={used_samples}"
            )
    ...
    # Rollback mechanism: if Stage2 cannot beat Stage1 by margin, keep Stage1 model.
    use_aux_refiner = best_fused_f1 >= (best_f1 + args.stage2_min_gain)
    if use_aux_refiner:
    ...
    if use_aux_refiner and best_aux_thr is not None:
        test_pred = fuse_pred(test_main_probs, test_aux_probs, main_thrs, best_aux_thr)
    else:
        test_pred = (test_main_probs > np.array(main_thrs, dtype=np.float32).reshape(1, -1)).sum(axis=1).astype(np.int64)
    ...
    metrics = {
        ...
        "val_macro_f1": best_fused_f1 if use_aux_refiner else best_f1,
        ...
        "best_stage1_val_macro_f1": best_f1,
        "best_stage2_val_macro_f1": best_fused_f1,
        "stage2_min_gain": args.stage2_min_gain,
        "use_aux_refiner": bool(use_aux_refiner),
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes MLP classification modelo5 (staged + fallback + hard-sample aux).")
    ...
    parser.add_argument("--lr-aux", type=float, default=1e-4)
    ...
    parser.add_argument("--stage2-min-gain", type=float, default=0.002)
    ...
    parser.add_argument("--patience-aux", type=int, default=5)
...
if __name__ == "__main__":
    ...
    resolve_dataset_runtime_paths(args, script_dir, "./cache_dataset_cls.npz")
    run(args)
