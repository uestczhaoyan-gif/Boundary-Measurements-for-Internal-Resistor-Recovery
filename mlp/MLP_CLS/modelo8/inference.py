...
def main():
    parser = argparse.ArgumentParser(description="Inference for 64Nodes MLP-Mixer classifier modelo8.")
    ...
    with torch.no_grad():
        for idx in sample_idx:
            ...
            aux_prob = float(torch.sigmoid(aux["aux_logit"]).cpu().numpy()[0])
            ...
            results.append(
                {
                    ...
                    "aux_ge2_prob": aux_prob,
                    "thresholds": thresholds,
                    "contrast_feat": aux["contrast_feat"].cpu().numpy()[0].tolist(),
                }
            )
            print(f"Sample index={idx} | pred={pred} | true={int(y[idx])} | aux_ge2_prob={aux_prob:.4f}")
