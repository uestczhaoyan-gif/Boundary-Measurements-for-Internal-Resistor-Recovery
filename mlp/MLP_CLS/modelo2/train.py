...
def run(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            # 2<->3 confusion penalty: add a mild extra supervision on the last threshold.
            adj_mask = yb >= 2
            if adj_mask.any():
        ...
        if ep % args.log_every == 0 or ep == 1:
            ...
            thrs, _ = search_thresholds_constrained(val_probs, val_true, step=args.thr_step)
    ...
    val_true = np.concatenate(val_true, axis=0)
    val_probs = 1.0 / (1.0 + np.exp(-val_logits / args.temp))
    thrs, best_val_f1 = search_thresholds_constrained(val_probs, val_true, step=args.thr_step)
