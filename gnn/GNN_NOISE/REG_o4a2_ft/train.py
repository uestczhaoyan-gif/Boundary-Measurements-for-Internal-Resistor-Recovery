...
DEFAULT_PRETRAINED_MODEL_PATH = "../../GNN_REG/o4a2/outputs/training_data64Nodes_2/model_last.pt"
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and self.add_noise:
            if self.noise_schedule == "fixed":
                noise_std = self.fixed_noise_std
            else:
                noise_std = self.noise_std_max * torch.rand(1).item()
            if noise_std <= 0:
                return x_sample, self.y_change[idx], self.y_delta[idx]
            ...
            if self.noise_mode == "uniform":
                ...
                noise = torch.empty_like(voltage_view).uniform_(-amplitude, amplitude)
            else:
                noise = torch.randn_like(voltage_view) * noise_std
            if self.noise_scope == "all":
...
def run(args):
    ...
    val_ds = RegDataset(x[va], y_change[va], y_delta[va], ext_nodes)
    test_ds = RegDataset(x[te], y_change[te], y_delta[te], ext_nodes)
    ...
    if args.pretrained_model_path:
        ...
        print(f"[Warm Start] loaded pretrained regressor from {args.pretrained_model_path}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes noisy fine-tuning for physics-informed GNN regression o4a2.")
    ...
    parser.add_argument("--noise-schedule", choices=["random", "fixed"], default="random")
