...
def evaluate_val(model, loader, device, args, mask_l1_weight):
    model.eval()
    total_loss = 0.0
    sum_all_abs = 0.0
    n_all = 0
    ...
    with torch.no_grad():
        for xb, ycb, ydb in loader:
            ...
            loss = loss_mse + mask_l1_weight * loss_mask
...
def run(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for xb, _ycb, ydb in train_loader:
            ...
            pred, aux = model(xb, return_aux=True)
            loss = F.mse_loss(pred, ydb) + current_mask_l1 * aux["mask_prob"].mean()
            opt.zero_grad()
            loss.backward()
            if args.grad_clip > 0:
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes physics-informed GNN regression o4a.")
    ...
    parser.add_argument("--lambda-mask-warmup-epochs", type=int, default=15)
    parser.add_argument("--mask-init-prob", type=float, default=0.35)
