...
def positive_edge_margin_loss(pred, ycb, margin):
    positive_mask = ycb > 0.5
    if not positive_mask.any():
        return pred.new_zeros(())
    return F.relu(margin - pred.abs())[positive_mask].mean()
...
def compute_o4a2_loss(pred, aux, ycb, ydb, mask_l1_weight, pos_weight, args):
    ...
    loss_margin = positive_edge_margin_loss(pred, ycb, args.true_edge_margin)
    loss = (
        loss_reg
        + args.mask_bce_weight * loss_mask
        + mask_l1_weight * loss_sparse
        + args.true_edge_margin_weight * loss_margin
    )
    return loss
...
def run(args):
    ...
    metrics = {
        ...
        "true_edge_margin": args.true_edge_margin,
        "true_edge_margin_weight": args.true_edge_margin_weight,
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes physics-informed GNN regression o5a.")
    ...
    parser.add_argument("--true-edge-margin", type=float, default=40.0)
    parser.add_argument("--true-edge-margin-weight", type=float, default=0.05)
