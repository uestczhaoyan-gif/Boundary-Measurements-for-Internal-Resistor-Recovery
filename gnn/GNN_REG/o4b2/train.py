...
def compute_decoupled_losses(
    ...
    pos_mask = ycb
    neg_mask = 1.0 - ycb
    loss_value_pos = (diff * pos_mask).sum() / (pos_mask.sum() + 1e-8)
    loss_value_bg = ((value_pred ** 2) * neg_mask).sum() / (neg_mask.sum() + 1e-8)
    loss = loss_mask + value_loss_weight * loss_value_pos + value_bg_weight * loss_value_bg
    return loss, loss_mask, loss_value_pos, loss_value_bg
...
def evaluate_val(model, loader, device, args, pos_weight):
    ...
    with torch.no_grad():
        for xb, ycb, ydb in loader:
            ...
            pred = combine_prediction(value_pred, aux)
            loss, _loss_mask, _loss_value_pos, _loss_value_bg = compute_decoupled_losses(
                value_pred,
                ycb,
                ydb,
                aux["mask_logits"],
...
def run(args):
    ...
    metrics = {
        ...
        "value_bg_weight": args.value_bg_weight,
        "hidden_dim": args.hidden_dim,
        "edge_hidden": args.edge_hidden,
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes physics-informed GNN regression o4b2.")
    ...
    parser.add_argument("--mask-pos-weight", type=float, default=15.0)
    parser.add_argument("--value-loss-weight", type=float, default=0.01)
    parser.add_argument("--value-bg-weight", type=float, default=0.001)
