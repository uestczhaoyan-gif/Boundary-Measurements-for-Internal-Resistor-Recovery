...
def combine_prediction(value_pred, aux):
    return aux["mask_prob"] * value_pred


def compute_decoupled_losses(value_pred, ycb, ydb, mask_logits, pos_weight, value_loss_weight):
    loss_mask = F.binary_cross_entropy_with_logits(mask_logits, ycb, pos_weight=pos_weight)
    diff = (value_pred - ydb) ** 2
    loss_value = (diff * ycb).sum() / (ycb.sum() + 1e-8)
    loss = loss_mask + value_loss_weight * loss_value
    return loss, loss_mask, loss_value
...
def search_best_count_threshold(model, loader, device, t_min=40, t_max=80, t_step=1):
    ...
    with torch.no_grad():
        for xb, ycb, _ in loader:
            value_pred, aux = model(xb.to(device), return_aux=True)
            pred = combine_prediction(value_pred, aux)
...
def evaluate_val(model, loader, device, args, pos_weight):
    model.eval()
    total_loss = 0.0
    sum_all_abs = 0.0
    n_all = 0
    ...
    with torch.no_grad():
        for xb, ycb, ydb in loader:
            ...
            pred = combine_prediction(value_pred, aux)
            loss, _loss_mask, _loss_value = compute_decoupled_losses(
                value_pred,
                ycb,
                ydb,
                aux["mask_logits"],
...
def run(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for xb, ycb, ydb in train_loader:
            xb, ycb, ydb = xb.to(device), ycb.to(device), ydb.to(device)
            value_pred, aux = model(xb, return_aux=True)
            loss, loss_mask, loss_value = compute_decoupled_losses(
                value_pred,
                ycb,
                ydb,
                aux["mask_logits"],
            ...
            tr_mask_loss += loss_mask.item() * xb.size(0)
            tr_value_loss += loss_value.item() * xb.size(0)
        tr_loss /= len(train_loader.dataset)
        tr_mask_loss /= len(train_loader.dataset)
        tr_value_loss /= len(train_loader.dataset)
        ...
        if ep % args.log_every == 0 or ep == 1:
            ...
            print(
                f"Epoch {ep:03d} | train_loss={tr_loss:.6f} | train_mask_bce={tr_mask_loss:.6f} "
                f"| train_value_mse={tr_value_loss:.6f} | val_loss={va_loss:.6f} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="64Nodes physics-informed GNN regression o4b.")
    ...
    parser.add_argument("--mask-pos-weight", type=float, default=55.0)
