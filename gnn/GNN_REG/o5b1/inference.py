...
def main():
    parser = argparse.ArgumentParser(description="Inference for 64Nodes physics-informed GNN regression o5b.")
    ...
    if Path(args.metrics_path).exists():
        ...
        args.hidden_dim = int(metrics.get("hidden_dim", args.hidden_dim))
        args.edge_hidden = int(metrics.get("edge_hidden", args.edge_hidden))
        args.edge_emb_dim = int(metrics.get("edge_emb_dim", args.edge_emb_dim))
        args.gat_heads = int(metrics.get("gat_heads", args.gat_heads))
        args.excitation_chunk_size = int(metrics.get("excitation_chunk_size", args.excitation_chunk_size))
        args.dropout = float(metrics.get("dropout", args.dropout))
        args.max_abs = float(metrics.get("max_abs", args.max_abs))
    if args.count_threshold is None and metrics:
