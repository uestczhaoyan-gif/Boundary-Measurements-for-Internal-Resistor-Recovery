...
try:
    ...
    raise ImportError(
        f"Failed to import torch dependencies for gnn/GNN_REG/o5b1/model/model.py: "
...
class PhysicsInformedGNNRegressor(nn.Module):
    def __init__(
        ...
        self.edge_embedding = nn.Embedding(out_dim, edge_emb_dim)
        self.edge_mlp = nn.Sequential(
            nn.Linear(hidden_dim * 3 + edge_emb_dim, edge_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
        ...
        self.register_buffer("edge_ids", torch.arange(len(resistor_edges), dtype=torch.long))
    ...
    def forward(self, x, return_aux=False):
        ...
        edge_emb = self.edge_embedding(self.edge_ids).unsqueeze(0).expand(bsz, -1, -1)
        edge_feat = torch.cat([hu, hv, torch.abs(hu - hv), edge_emb], dim=-1)
