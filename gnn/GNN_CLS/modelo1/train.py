...
def search_thresholds_constrained_weighted(val_probs, val_true, step=0.01, fixed_t2=None, penalty_32=0.12, bonus_r3=0.06, bonus_r2=0.05):
...
def run(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if ep % args.log_every == 0 or ep == 1:
            ...
            thrs, val_f1 = search_thresholds_constrained_weighted(
                val_probs,
                val_true,
                step=args.thr_step,
    ...
    val_true = np.concatenate(val_true, axis=0)
    val_probs = 1.0 / (1.0 + np.exp(-val_logits / args.temp))
    thrs, best_val_f1 = search_thresholds_constrained_weighted(
        val_probs,
        val_true,
        step=args.thr_step,
        fixed_t2=args.fixed_t2,
...
if __name__ == "__main__":
    ...
    resolve_dataset_runtime_paths(args, script_dir, "./cache_dataset_cls_v2.npz")
    run(args)
