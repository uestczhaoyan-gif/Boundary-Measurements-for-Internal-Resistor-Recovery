...
from model.model_cls import PhysicsInformedGNNClassifier
from model.model_reg import PhysicsInformedGNNRegressor
...
def ensure_cache_npz(cache_path, csv_path, builder_module_path, module_name):
    ...
    print(f"[Runtime Mv1] rebuilding missing cache -> {cache_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Unified GNN_FULL/Mv1 inference with GNN_CLS + GNN_REG.")
    ...
    parser.add_argument("--cls-out-dir", default="outputs/cls")
    parser.add_argument("--reg-out-dir", default="outputs/reg")
    parser.add_argument("--cls-cache-dir", default="cache")
    parser.add_argument("--reg-cache-dir", default="cache")
    ...
    cls_root = resolve_output_dir(args.cls_out_dir, script_dir)
    reg_root = resolve_output_dir(args.reg_out_dir, script_dir)
    cls_cache_root = resolve_output_dir(args.cls_cache_dir, script_dir)
    reg_cache_root = resolve_output_dir(args.reg_cache_dir, script_dir)

    cls_out = resolve_default_artifact_path(cls_root, "model_last.pt", dataset_tag, data_path).parent
    reg_out = resolve_default_artifact_path(reg_root, "model_last.pt", dataset_tag, data_path).parent
    ...
    cls_cache = resolve_default_artifact_path(cls_cache_root, "cache_dataset_cls_graphattn.npz", dataset_tag, data_path)
    reg_cache = resolve_default_artifact_path(reg_cache_root, "cache_dataset_reg_graphattn.npz", dataset_tag, data_path)
    cls_cache = ensure_cache_npz(cls_cache, data_path, script_dir / "train_cls.py", "mv1_train_cls_cache")
    reg_cache = ensure_cache_npz(reg_cache, data_path, script_dir / "train_reg.py", "mv1_train_reg_cache")
    ...
    report_lines = [
        ...
        f"- Near-Miss: `enabled={args.enable_near_miss}`, `ratio={args.near_miss_ratio:.2f}`",
        f"- **CMEI={cmei:.2f}**",
        "",
        "## Confusion Matrix",
        "```text",
    ...
    for row in detail_rows:
        print(f"Sample index={row['sample_index']} | pred_k={row['pred_k']} | true_k={row['true_k']}")
