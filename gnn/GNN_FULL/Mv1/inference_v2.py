...
_GNN_ROOT = Path(__file__).resolve().parents[2]
...
_BOOTSTRAP_RESULTS = bootstrap_vendor_paths(_PROJECT_ROOT)
...
def ensure_cache_npz(cache_path, csv_path, builder_module_path, module_name):
    cache_path = Path(cache_path)
    if cache_path.exists():
        return cache_path
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    build_dataset = load_module_attr(module_name, builder_module_path, "build_dataset")
    print(f"[Runtime Mv1 v2] rebuilding missing cache -> {cache_path}")
    build_dataset(Path(csv_path), cache_path)
    if not cache_path.exists():
        raise FileNotFoundError(f"Failed to build cache: {cache_path}")
    return cache_path
...
def apply_near_miss(
    ...
    if float(reg_prob[weakest]) > prob_guard or float(reg_abs[weakest]) > abs_guard:
        return selected
...
def main():
    parser = argparse.ArgumentParser(description="Unified GNN_FULL/Mv1 inference v2 with guarded near-miss only.")
    parser.add_argument("--data-path", default=DEFAULT_MAIN_DATA_PATH)
    parser.add_argument("--dataset-tag", default="")
    parser.add_argument("--out-dir", default="outputs/inference_v2")
    ...
    parser.add_argument("--near-miss-prob-guard", type=float, default=0.85)
    parser.add_argument("--near-miss-abs-guard", type=float, default=45.0)
    ...
    cls_cache = ensure_cache_npz(cls_cache, data_path, script_dir / "train_cls.py", "mv1_train_cls_cache_v2")
    reg_cache = ensure_cache_npz(reg_cache, data_path, script_dir / "train_reg.py", "mv1_train_reg_cache_v2")
    ...
    x_cls = standardize_gnn(x_cls, cls_std["mean"], cls_std["std"], cls_std["ext_nodes"].astype(np.int64))
    ...
    x_reg = standardize_gnn(x_reg, reg_std["mean"], reg_std["std"], reg_std["ext_nodes"].astype(np.int64))

    if not np.array_equal(y_cls, y_change.sum(axis=1).astype(np.int64)):
        raise RuntimeError("Mv1 cls/reg cached labels are inconsistent.")
    ...
    if "mask_init_prob" in reg_metrics:
        reg_kwargs["mask_init_prob"] = float(reg_metrics["mask_init_prob"])
    if "edge_emb_dim" in reg_metrics:
        reg_kwargs["edge_emb_dim"] = int(reg_metrics["edge_emb_dim"])

    reg_model = PhysicsInformedGNNRegressor(**reg_kwargs).to(device)
    ...
    raw_pred_counts = predict_count_from_coral(cls_probs_all, cls_thresholds)
    ...
    final_pred_counts = np.zeros_like(raw_pred_counts)
    correct_id_total = 0
    true_id_total = int(y_change_test.sum())

    for i in range(len(test_idx)):
        k_pred = int(raw_pred_counts[i])
        ...
        if args.enable_near_miss:
            topk_ids = apply_near_miss(
                ...
                prob_guard=args.near_miss_prob_guard,
                abs_guard=args.near_miss_abs_guard,
            )

        final_pred_counts[i] = len(topk_ids)
        if topk_ids:
    ...
    num_accuracy = float((final_pred_counts == y_cls_test).mean())
    cm = confusion(final_pred_counts, y_cls_test, num_classes=4)
    f1 = macro_f1(cm)
    ...
    metrics = {
        ...
        "raw_cls_num_accuracy": float((raw_pred_counts == y_cls_test).mean()),
        ...
        detail_rows.append(
            {
                ...
                "raw_cls_k": int(raw_pred_counts[local_idx]),
                "pred_k": int(final_pred_counts[local_idx]),
                "true_ids": true_ids,
                "pred_ids": pred_ids,
    ...
    report_lines = [
        ...
        f"- Near-Miss: `enabled={args.enable_near_miss}`, `ratio={args.near_miss_ratio:.2f}`, `prob_guard={args.near_miss_prob_guard:.2f}`, `abs_guard={args.near_miss_abs_guard:.1f}`",
        f"- **CMEI={cmei:.2f}**",
        "",
        "## Confusion Matrix",
        "```text",
    ...
    for row in detail_rows:
        report_lines.append(
            f"- Sample `{row['sample_index']}`: true_k=`{row['true_k']}` raw_cls_k=`{row['raw_cls_k']}` "
            f"pred_k=`{row['pred_k']}` true_ids=`{row['true_ids']}` pred_ids=`{row['pred_ids']}`"
        )
    ...
    print("[Raw Metrics]")
    print(
        f"raw_cls_num_accuracy={float((raw_pred_counts == y_cls_test).mean()):.4f} | "
        f"num_accuracy={num_accuracy:.4f} | macro_f1={f1:.4f} | "
        f"id_recall={id_recall:.4f} | mse_all_edges={mse:.4f}"
    )
    ...
    for row in detail_rows:
        print(
            f"Sample index={row['sample_index']} | raw_cls_k={row['raw_cls_k']} | "
            f"pred_k={row['pred_k']} | true_k={row['true_k']}"
        )
