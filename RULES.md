...
## 1. 项目简介
- 问题本质：
  - 在某种固定拓扑下的电阻网络中，当内部某些电阻阻值发生变化，只能通过外部节点的激励与测量去重建内部变化状态。
- 当前仓库的标准 clean 主线实现是：
  - `8x8` 网格
  - `64` 节点
  - `112` 条电阻边
...
## 13. Git 实验分支与最佳版本 Tag 规范
- `main` 只用于保存当前稳定、可回退、可作为正式路线的项目状态。
- 日常实验默认不直接在 `main` 上推进。
- 需要开启新实验时，默认新建 Git 分支，分支名前缀统一为：
  - `codex/`
- 推荐分支命名格式为：
  - `codex/<模块>-<版本>-<目的>`
- 推荐示例：
...
- 当某次实验结果准备升级为“当前最佳版本”时，必须同步完成以下动作：
  - 将稳定代码整理进入 `main`
  - 更新 `CURRENT_BEST.md`
  - 追加更新根目录 `README.md / Log.md`
  - 追加更新对应子目录 `README.md / Log.md`
  - 新增一个可读、可回退的 Git tag
- 推荐 tag 命名格式为：
  - `best-<模块>-<版本>-<日期>`
- 推荐示例：
...
- 只有满足以下条件时，才建议新增“当前最佳版本” tag：
  - 已有明确指标、对比结果或工程结论
  - 已写入对应目录的 `README.md / Log.md`
  - 已更新 `CURRENT_BEST.md`
  - 已确认该版本可作为后续恢复锚点
- Git tag 原则上只新增，不覆盖、不删除旧 tag。
- 如果某次改动仍处于探索、排错、半成品状态，则停留在实验分支，不进入 `main`，也不打“当前最佳版本” tag。
