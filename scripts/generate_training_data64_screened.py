...
_NUMPY_VENDOR = Path(__file__).resolve().parents[1] / "inverse_identifiability" / ".vendor"
if _NUMPY_VENDOR.exists() and str(_NUMPY_VENDOR) not in sys.path:
    sys.path.insert(0, str(_NUMPY_VENDOR))
...
def build_excitations(ext_nodes):
    ...
    excitations.extend([(0, 63), (7, 56), (3, 60), (31, 32)])
    return excitations
...
def node_boundary_distance(node_id):
    r = node_id // GRID_SIZE
    c = node_id % GRID_SIZE
    return min(r, c, GRID_SIZE - 1 - r, GRID_SIZE - 1 - c)


def build_internal_resistor_ids(edges, min_node_boundary_distance=2):
    ids = []
    for rid, (n1, n2) in enumerate(edges):
        if node_boundary_distance(n1) >= min_node_boundary_distance and node_boundary_distance(n2) >= min_node_boundary_distance:
            ids.append(rid)
    return ids


def sample_changes(k, rng, seen, min_ratio, max_ratio, internal_ids, skip_internal_opposite_pairs):
    if k == 0:
        return [], False
    while True:
        ...
        for rid in rids:
            ...
            if rid in internal_ids:
                internal_signs.append(sign)

        invalid = False
        if skip_internal_opposite_pairs and len(internal_signs) >= 2:
            invalid = (min(internal_signs) < 0.0) and (max(internal_signs) > 0.0)
...
def main():
    parser = argparse.ArgumentParser(description="Generate screened 8x8 resistor-grid data by rejecting internally opposite-sign ambiguous samples.")
    parser.add_argument("--output", default="data/training_data64Nodes_2_screened.csv", help="CSV output path")
    parser.add_argument("--meta-output", default="data/training_data64Nodes_2_screened_meta.json", help="Metadata JSON path")
    ...
    parser.add_argument("--current-a", type=float, default=0.01, help="Current source amplitude (A)")
    ...
    parser.add_argument("--internal-boundary-distance", type=int, default=2, help="A resistor is internal only if both endpoint nodes are at least this far from the boundary.")
    parser.add_argument("--skip-internal-opposite-pairs", action="store_true")
    parser.add_argument("--no-skip-internal-opposite-pairs", dest="skip_internal_opposite_pairs", action="store_false")
    parser.set_defaults(skip_internal_opposite_pairs=True)
    ...
    internal_ids = set(build_internal_resistor_ids(edges, min_node_boundary_distance=args.internal_boundary_distance))
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for k in [0, 1, 2, 3]:
            ...
            while produced < counts[k]:
                changes, skipped = sample_changes(
                    k,
                    rng,
                    seen_changed,
                    args.min_ratio,
                    args.max_ratio,
                    internal_ids,
                    args.skip_internal_opposite_pairs,
                )
                if skipped:
                    skipped_internal_ambiguous += 1
                    continue
                if k > 0 and not changes:
    ...
    meta = {
        ...
        "screening_rule": {
            "enabled": bool(args.skip_internal_opposite_pairs),
            "internal_boundary_distance": args.internal_boundary_distance,
            "internal_resistor_ids": sorted(internal_ids),
            "description": "If changed internal resistors contain both positive and negative signs, reject and resample to keep total combos unchanged.",
        },
        "skipped_internal_ambiguous": skipped_internal_ambiguous,
        "duplicate_resamples": duplicate_resamples,
        "note": "Direct Kirchhoff solves with ambiguity-screened 10mA dataset; total combo count preserved by rejection sampling.",
    }
    meta_path.write_text(json.dumps(meta, indent=2), encoding="utf-8")

    print(f"Generated rows: {row_id}")
    print(f"Generated combos: {combo_id}")
    print(f"Skipped ambiguous internal opposite-sign combos: {skipped_internal_ambiguous}")
    print(f"Duplicate resamples: {duplicate_resamples}")
