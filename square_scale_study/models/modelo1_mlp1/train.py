...
from models.loss_utils import (
    balanced_bce_with_logits_loss,
    pairwise_ranking_hinge_loss,
    weighted_two_part_mse_loss,
)
...
def predict_arrays(model: Modelo1MLP1Regressor, x: np.ndarray, batch_size: int, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    score_rows = []
    value_rows = []
    with torch.no_grad():
...
def evaluate_split(model: Modelo1MLP1Regressor, x: np.ndarray, y: np.ndarray, k: int, batch_size: int, device: torch.device) -> dict:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train square_scale_study modelo1_mlp1.")
    ...
    parser.add_argument("--score-bce-weight", type=float, default=1.0)
    ...
    parser.add_argument("--score-neg-weight", type=float, default=0.15)
    parser.add_argument("--ranking-margin", type=float, default=1.0)
    parser.add_argument("--value-changed-weight", type=float, default=3.0)
    parser.add_argument("--value-unchanged-weight", type=float, default=0.05)
...
def train_from_meta(args: argparse.Namespace) -> dict:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss_score_bce, score_parts = balanced_bce_with_logits_loss(
                score_logits,
                yb,
                pos_weight=args.score_pos_weight,
                neg_weight=args.score_neg_weight,
            )
            loss_ranking = pairwise_ranking_hinge_loss(score_logits, yb, margin=args.ranking_margin)
            loss_value, value_parts = weighted_two_part_mse_loss(
                value_pred,
                yb,
            ...
            loss = (
                args.score_bce_weight * loss_score_bce
                + args.score_rank_weight * loss_ranking
                + args.value_loss_weight * loss_value
            )
            ...
            seen += batch
            loss_book["loss_score_bce"] += float(loss_score_bce.item()) * batch
            loss_book["loss_score_pos"] += float(score_parts["loss_score_pos"].item()) * batch
            loss_book["loss_score_neg"] += float(score_parts["loss_score_neg"].item()) * batch
            loss_book["loss_ranking"] += float(loss_ranking.item()) * batch
            loss_book["loss_changed"] += float(value_parts["loss_changed"].item()) * batch
            loss_book["loss_unchanged"] += float(value_parts["loss_unchanged"].item()) * batch
    ...
    payload = {
        ...
        "model_config": {
            ...
            "score_pos_weight": args.score_pos_weight,
            "score_neg_weight": args.score_neg_weight,
