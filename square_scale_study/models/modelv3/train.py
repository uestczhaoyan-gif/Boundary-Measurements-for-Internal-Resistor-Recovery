...
from models.loss_utils import (
    ...
    weighted_two_part_smooth_l1_loss,
)
...
def predict_arrays(model: ModelV3Regressor, x: np.ndarray, batch_size: int, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    score_rows = []
    value_rows = []
    with torch.no_grad():
...
def evaluate_split(model: ModelV3Regressor, x: np.ndarray, y: np.ndarray, k: int, batch_size: int, device: torch.device) -> dict:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train square_scale_study modelv3.")
    ...
    parser.add_argument("--score-neg-weight", type=float, default=0.25)
    parser.add_argument("--ranking-margin", type=float, default=1.0)
    parser.add_argument("--value-changed-weight", type=float, default=2.0)
    parser.add_argument("--value-unchanged-weight", type=float, default=0.2)
...
def train_from_meta(args: argparse.Namespace) -> dict:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss = args.score_bce_weight * loss_score_bce + args.score_rank_weight * loss_ranking + loss_value
            ...
            seen += batch_size
            loss_book["loss_score_bce"] += float(loss_score_bce.item()) * batch_size
            loss_book["loss_score_pos"] += float(score_parts["loss_score_pos"].item()) * batch_size
            loss_book["loss_score_neg"] += float(score_parts["loss_score_neg"].item()) * batch_size
