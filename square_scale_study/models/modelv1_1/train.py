...
from models.loss_utils import weighted_two_part_smooth_l1_loss
...
def predict_array(model: ModelV11Regressor, x: np.ndarray, batch_size: int, device: torch.device) -> np.ndarray:
    model.eval()
    preds = []
    with torch.no_grad():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train square_scale_study modelv1.1.")
    ...
    parser.add_argument("--changed-weight", type=float, default=1.0)
    parser.add_argument("--unchanged-weight", type=float, default=0.10)
...
def train_from_meta(args: argparse.Namespace) -> dict:
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        total_loss = 0.0
        loss_changed_sum = 0.0
        loss_unchanged_sum = 0.0
        seen = 0
        ...
        for xb, yb in train_loader:
            ...
            yb = yb.to(device)
            pred = model(xb)
            loss, parts = weighted_two_part_smooth_l1_loss(
                pred,
                yb,
            ...
            loss_changed_sum += float(parts["loss_changed"].item()) * batch_size
            loss_unchanged_sum += float(parts["loss_unchanged"].item()) * batch_size
            seen += batch_size
        ...
        entry = {
            ...
            "loss_changed": loss_changed_sum / max(seen, 1),
            "loss_unchanged": loss_unchanged_sum / max(seen, 1),
