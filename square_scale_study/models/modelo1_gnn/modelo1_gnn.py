...
class ResidualGraphBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        conv_out = self.conv(h, edge_index)
        h = self.norm1(h + conv_out)
        ff_out = self.ff(h)
...
class ExcitationAttentionPool(nn.Module):
    ...
    def forward(self, h: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        # h: [B, E, N, H]
        excitation_context = h.mean(dim=2)
        attn_logits = self.score(excitation_context).squeeze(-1)
        attn_weights = torch.softmax(attn_logits, dim=1)
        weighted = torch.einsum("be,benh->bnh", attn_weights, h)
        mean_pool = h.mean(dim=1)
        max_pool = h.max(dim=1).values
        fused = torch.cat([weighted, mean_pool, max_pool], dim=-1)
        return fused, {
...
class Modelo1GNNRegressor(nn.Module):
    def __init__(
        self,
