...
def predict_arrays(model: Modelo1GNNRegressor, x: np.ndarray, batch_size: int, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    score_rows = []
    value_rows = []
    with torch.no_grad():
...
def evaluate_split(model: Modelo1GNNRegressor, x: np.ndarray, y: np.ndarray, k: int, batch_size: int, device: torch.device) -> dict:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train square_scale_study modelo1_gnn.")
