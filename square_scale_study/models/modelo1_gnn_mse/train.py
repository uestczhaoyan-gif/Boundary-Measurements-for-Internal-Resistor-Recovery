...
def predict_arrays(model: Modelo1GNNMSERegressor, x: np.ndarray, batch_size: int, device: torch.device) -> np.ndarray:
    model.eval()
    value_rows = []
    with torch.no_grad():
...
def evaluate_split(model: Modelo1GNNMSERegressor, x: np.ndarray, y: np.ndarray, k: int, batch_size: int, device: torch.device) -> dict:
...
def default_out_dir(meta_path: Path) -> Path:
    ...
    return PROJECT_ROOT / "outputs_modelo1_gnn_mse" / run_dir_name(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train square_scale_study modelo1_gnn_mse.")
